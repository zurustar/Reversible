...
- **スライド(タイ)**: `step.slide === true` のステップは**次のノートへスライド(タイ)**する(UIヒント「▶=次へスライド」)。あるノートがレガートになる条件は**直前ステップの slide フラグ**(自分自身ではない)。
  - **音の長さ**: スライドするノートは減衰させず**1ステップ分ピークを保持(サステイン)**し、次のステップのノートへ繋ぐ。よって「2ノート連続+1音目 slide」は**8分音符相当の1本の音**になる。タイの終端ノート(自身は slide=false)で通常のディケイ解放を行う。次のノートが来なければ保持後に解放する(音が鳴りっぱなしにならないよう1ステップで打ち切り)。
  - **繋ぎ方**: レガート入場のノートはゼロからの再アタック(クリック)をせずピークから継続し、ピッチのみ前ノートから固定時間でグライド(~10..150ms)。フィルターエンベロープはスライド時は再トリガしない。
  - 実装: `BasslineVoice` は「直前ノートが slide を要求したか」(`slideInto`)を状態で保持。ステップ長 `stepDur` は Scheduler / オフラインレンダラから渡す(bpm 由来)。
